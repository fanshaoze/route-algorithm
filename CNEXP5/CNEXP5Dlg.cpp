...
void CCNEXP5Dlg::OnStartClickedButton()
{
	...
	{		
		...
		if ((IfInfo[i].adhandle = pcap_open((LPSTR)(LPCTSTR)IfInfo[i].DeviceName, // 设备名 
			65536,           // 最大包长度 
		...
		{
			// 错误，显示错误信息 
			sprintf_s(strbuf, "接口未能打开。WinPcap不支持%s。",
				IfInfo[i].DeviceName);
			CString cs1;
	...
	// 开启数据包捕获线程，获取本地接口的MAC地址，线程数目为网卡个数 
		...
		/*if (k != IfCount)
		{
			MessageBox(_T("至少有一个接口的MAC地址没能得到！"));
			PostMessage(WM_QUIT, 0, 0);
		}*/
		...
		for (i = 0; i < IfCount; i++)
		{
			...
			Logger.InsertString(-1, (_T("MAC地址：") + MACntoa(IfInfo[i].MACAddr)));
			for (j = 0; j < IfInfo[i].ip.GetSize(); j++)
			{
				Logger.InsertString(-1, (_T("IP地址：") + IPntoa(IfInfo[i].ip[j].IPAddr)));
			}
		}
	...
	*pkt_data)
{
	...
	SendPacket_t  sPacket;
	IPf = (IPFrame_t *) pkt_data;
	pDlg->Logger.InsertString(-1,  _T(" 收到 IP 数 据 包 :"  + 
